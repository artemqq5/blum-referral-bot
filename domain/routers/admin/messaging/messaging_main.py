...
from domain.notification.NotificationUser import NotificationUser
from domain.routers.admin.messaging.MessagingTools import MessagingTools
from domain.states.MessagingState import MessagingState
from presentation.keyboard_admin.kb_admin import NotificationForUsers
from presentation.keyboard_admin.kb_messaging import *

router = Router()


@router.callback_query(NotificationForUsers.filter())
async def messaging(callback: CallbackQuery, state: FSMContext, i18n: I18nContext):
    await state.clear()
...
async def messaging_back(callback: CallbackQuery, state: FSMContext, i18n: I18nContext):
    await messaging(callback, state, i18n)
...
async def preview_send(callback: CallbackQuery, state: FSMContext, i18n: I18nContext, bot: Bot):
    ...
    if data.get('telegram_id', None):
        result = await NotificationUser.push_individual_user(data, bot, data.get('telegram_id'), i18n)
    else:
        result = await NotificationUser.push_all_users(data, bot, i18n)
