...
ADMIN-POST-CHOICE_CHANNEL = Выберите канал для публикации 📢
...
ADMIN-POST-SET_DESC_ERROR = Длина описания <b>{$size}</b>, но должна быть не более 1000 ❗️
ADMIN-POST-SET_MEDIA = Медиа файл (видео\картинка): 🖼️🎥
ADMIN-POST-SET_MEDIA_ERROR = Неверный формат, отправьте (Фото\Видео) в сжатом формате: ❌
ADMIN-POST-SET_BUTTON = Добавить кнопку к посту? 🔘
ADMIN-POST-SET_BUTTON_TEXT = Текст для кнопки (желательно 20-30 символов): ✏️
ADMIN-POST-BUTTON_TEXT_ERROR = Укоротите хотя бы до 50 символов, сейчас {$count} ❗️
ADMIN-POST-SET_BUTTON_URL = Ссылка для кнопки: 🔗
ADMIN-POST-BUTTON_URL_ERROR = Ссылка имеет неправильный формат, должна начинаться с https:// и содержать сайт или настоящий контакт в ТГ и т.д. ❌
...
ADMIN-POST-SEND_ERROR_DB = Не удалось отправить (database) ❌
